    ...
    <div aria-hidden="true">
      ...
      <div className="mt-5 flex items-start gap-2.5">
        <img
          ...
          alt=""
          aria-hidden="true"
          className="w-16 h-16 rounded-[12px] flex-shrink-0"
          style={{ boxShadow: "0 1px 2px 0 rgba(60,64,67,.3), 0 1px 3px 1px rgba(60,64,67,.15)" }}
        />
        <div className="min-w-0">
          <p className="text-base text-ink truncate font-roboto">{game.title}</p>
          <p className="text-sm text-ink-dim truncate font-roboto">{game.tags.slice(0, 2).join(" · ")}</p>
          <p className="text-sm text-ink-dim flex items-center gap-0.5 mt-0.5 font-roboto">
            {game.rating.toFixed(1)}<span className="text-ink-dim">★</span>
          </p>
        </div>
      </div>
    </div>
    ...
    <section className="mt-10" aria-label={title}>
      ...
      <div className="hidden md:block relative group/rec">
        {canGoPrev && (
          <button
            ...
            className="absolute -left-5 top-[46%] -translate-y-1/2 z-10 w-[52px] h-[52px] bg-white rounded-full shadow-md flex items-center justify-center text-ink-dim hover:text-ink opacity-0 group-hover/rec:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-chart-green/40 transition-opacity duration-200 disabled:pointer-events-none"
          >
      ...
      <div className="hidden lg:block md:hidden relative group/mid">
        <button
          ...
          aria-label={`Previous ${title} games`}
          className="absolute left-0 top-[46%] -translate-y-1/2 z-10 w-[52px] h-[52px] bg-white rounded-full shadow-md flex items-center justify-center text-ink-dim hover:text-ink opacity-0 group-hover/mid:opacity-100 focus-visible:opacity-100 focus-visible:outline-none transition-opacity duration-200"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round">
            <path d="M15 18l-6-6 6-6" />
          </svg>
        ...
        <button
          ...
          aria-label={`Next ${title} games`}
          className="absolute right-0 top-[46%] -translate-y-1/2 z-10 w-[52px] h-[52px] bg-white rounded-full shadow-md flex items-center justify-center text-ink-dim hover:text-ink opacity-0 group-hover/mid:opacity-100 focus-visible:opacity-100 focus-visible:outline-none transition-opacity duration-200"
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round">
            <path d="M9 18l6-6-6-6" />
          </svg>
